...
This Dockerfile builds a Fedora 34 based container, capable to use `systemd`, mainly for Ansible role testing.
...
3. Run a container:
   ...
   ```shell
   docker run -d --privileged --volume=/sys/fs/cgroup:/sys/fs/cgroup:ro fedora34-systemd-ansible:latest
   ```
